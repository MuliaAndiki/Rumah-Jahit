...
async function getUniversalAccessToken(): Promise<string | undefined> {
  ...
  try {
    ...
    return store.get(APP_SESSION_COOKIE_KEY)?.value;
  } catch {
    return undefined;
  }
}
...
async function clientCoreFetchResponse<T>(
  ...
  if (options.withAuth) {
    if (_tokenProvider) {
      ...
      accessToken = await getUniversalAccessToken();
    }
  }

  const endpoint = await buildApiUrl(path);

  Logger.request(method, endpoint, { ...buildBaseHeaders(accessToken), ...extraHeaders }, body);
  ...
  Logger.response(method, endpoint, res.status, json?.data);
